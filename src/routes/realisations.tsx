...
import archHouseRoof from "@/assets/decor/arch-house-roof.webp";
import archCluster from "@/assets/decor/arch-cluster.webp";

export const Route = createFileRoute("/realisations")({
  head: () => ({
    meta: [
      { title: "Chantiers livrés à Agadir & Maroc | Lahlou Workers" },
      {
        name: "description",
        content:
          "Résidences MISSIMI, immeuble R+5 post-tension, VRD cimenterie Tan-Tan, villas avec piscines, bureaux : ce que nos équipes ont construit.",
      },
      { property: "og:title", content: "Réalisations | Lahlou Workers" },
      {
        property: "og:description",
        content: "Gros œuvre, VRD, aménagement : chantiers livrés partout au Maroc.",
      },
    ],
  }),
...
function Realisations() {
  ...
  // Memoized: without this, every parent re-render hands StackedProjects a new
  // array identity and tears down its live GSAP pins mid-scroll.
  const allProjects = useMemo(() => [...(adminProjects ?? []), ...projects], [adminProjects]);
  const visible = useMemo(
    () => (active === "Tous" ? allProjects : allProjects.filter((p) => p.type === active)),
    [active, allProjects],
  );
    ...
    <PageShell>
      <section className="relative mx-auto max-w-[1440px] overflow-hidden px-5 pb-10 pt-40 lg:px-8 lg:pt-48">
        <img
          ...
          alt=""
          aria-hidden
          className="pointer-events-none absolute right-6 top-36 w-20 -rotate-6 opacity-90 sm:w-28 lg:right-10 lg:top-44 lg:w-32"
        />
        <FadeUp>
          <Eyebrow label="Réalisations" />
          <h1 className="mt-6 font-display text-[32px] font-bold leading-[0.95] lg:text-[64px]">
            Chantiers livrés.
          </h1>
        </FadeUp>
      ...
      <section className="relative mx-auto max-w-[1440px] px-5 pb-24 lg:px-8 lg:pb-36">
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <img
            ...
            alt=""
            aria-hidden
            className="absolute -right-40 top-0 hidden w-[720px] opacity-[0.22] mix-blend-multiply lg:block"
          />
        </div>
        {adminProjects !== null && (
          <StackedProjects
