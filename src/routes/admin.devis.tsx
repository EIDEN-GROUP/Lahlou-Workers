...
export const Route = createFileRoute("/admin/devis")({
  head: () => ({ meta: [{ title: "Devis — Admin Lahlou Workers" }] }),
...
function AdminDevis() {
  ...
  return (
    <AdminShell>
      ...
      <p className="mt-2 text-sm text-muted-foreground">Demandes reçues via le formulaire en quatre étapes.</p>
          ...
          {items.map(item => (
            <div key={item.id} className="border border-border bg-background p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-semibold">{item.name || "—"} <span className="text-muted-foreground">— {item.company || "particulier"}</span></p>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
                <div><span className="text-muted-foreground">Besoin</span><p className="font-medium">{item.need || "—"}</p></div>
                <div><span className="text-muted-foreground">Ville</span><p className="font-medium">{item.city || "—"}</p></div>
                <div><span className="text-muted-foreground">Surface</span><p className="font-medium">{item.surface ? `${item.surface} m²` : "—"}</p></div>
                <div><span className="text-muted-foreground">Équipe</span><p className="font-medium">{item.crewSize} personnes</p></div>
              </div>
              {item.trades.length > 0 && <p className="mt-3 text-sm text-muted-foreground">Métiers : {item.trades.join(", ")}</p>}
            </div>
          ))}
        </div>
      )}
