...
export function Wordmark({
  ...
  return (
    <a
      ...
      className={`font-display font-bold leading-[0.82] tracking-tight ${inverse ? "text-background" : "text-foreground"} ${className ?? "text-[15px]"}`}
    >
      <span className="block">
        lah<span className={accentClassName}>l</span>ou
      </span>
...
export function SectionHead({
  ...
  return (
    <div className={center ? "mx-auto max-w-3xl text-center" : ""}>
      ...
      <h2 className="mt-6 font-display text-[28px] font-bold uppercase leading-[0.95] lg:text-[52px]">
        {parts[0]}
        {red && <span className="text-primary">{red}</span>}
        {parts[1]}
      </h2>
    </div>
  );
...
export function SiteHeader({ variant = "light" }: { variant?: HeaderVariant }) {
  ...
  // Circle-spread origin (toggle-button centre + radius covering the viewport).
  // Measured on every toggle so desktop & mobile both wipe from the button.
  const [wipe, setWipe] = useState({ x: 0, y: 0, r: 0, on: true });
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 bg-transparent">
      <div className="mx-auto flex h-[88px] max-w-[1440px] items-center justify-between px-5 lg:px-8">
        <div
          className={`flex items-center justify-center rounded-full transition-all duration-300 ${scrolled && !menuOpen ? "bg-primary px-5 py-2.5 shadow-lg" : "px-0 py-0"}`}
        >
          <Wordmark
            inverse={scrolled && !menuOpen ? true : inverse}
            accentClassName={scrolled && !menuOpen ? "text-background" : "text-primary"}
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.nav
            id="menu-principal"
            initial={
              wipe.on ? { clipPath: `circle(0px at ${wipe.x}px ${wipe.y}px)` } : { opacity: 0 }
            }
            animate={
              wipe.on
                ? { clipPath: `circle(${wipe.r}px at ${wipe.x}px ${wipe.y}px)` }
                : { opacity: 1 }
            }
            exit={
              wipe.on
              ...
              >
                {navLinks.map(([a, b], i) => (
                  <motion.a
                    ...
                    href={b}
                    variants={menuItem}
                    className="group flex items-baseline gap-5 border-b border-background/10 py-4 transition-colors hover:text-primary sm:py-5"
                    onClick={() => setMenu(false)}
                  >
                    <span className="font-display text-xs font-bold text-background/40 transition-colors group-hover:text-primary">
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <span className="font-display text-[9vw] font-bold leading-[0.95] sm:text-[64px] lg:text-[80px]">
                      {a}
                    </span>
                  </motion.a>
                ))}
                <motion.div variants={menuItem} className="mt-10">
                  <SplitButton href="/devis" onClick={() => setMenu(false)}>
                    Demander une équipe
                  </SplitButton>
                </motion.div>
              </motion.div>
              ...
              <div className="flex items-center gap-3">
                {socials.map(([label, href, Icon]) => (
                  <a
                    ...
                    rel="noreferrer"
                    aria-label={label}
                    className="flex h-9 w-9 items-center justify-center rounded-full border border-background/25 transition-colors hover:border-background hover:text-primary"
                  >
      ...
      <div className="mx-auto max-w-[1440px] px-5 py-12 lg:px-8 lg:py-16">
        <div className="grid gap-12 border-t border-background/20 pt-12 lg:grid-cols-12">
          <div className="lg:col-span-5">
            <Wordmark inverse className="text-[22px]" />
            <p className="mt-5 max-w-xs text-sm leading-relaxed text-muted-foreground">
              Entreprise générale à Agadir : gros œuvre, aménagement de bureaux, génie civil &amp;
              VRD, rénovation. Résidences, bureaux et villas, de la conception à la livraison.
            </p>
          ...
          <div className="grid gap-10 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
            ...
            <div>
              ...
              <div className="mt-5 grid gap-3 text-sm">
                ...
                <a href="/devis">Demander une équipe</a>
              </div>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">
              ...
              <p className="mt-3 text-xs text-muted-foreground">{SITE.hours}</p>
            </div>
          </div>
        </div>
        <div className="flex flex-col gap-3 pt-6 text-xs text-muted-foreground sm:flex-row sm:justify-between">
          <span>
            © 2026 {SITE.name} - ICE {SITE.ice} · RC {SITE.rc}
          </span>
          <span>Gros œuvre · Travaux d’aménagement</span>
        </div>
...
export function WhatsAppButton() {
  ...
  useEffect(() => {
    const clientNum =
      ...
      "";
    if (clientNum) setHref(`https://wa.me/${clientNum.replace(/\D/g, "")}`);
    else setHref(whatsappLink("Bonjour Lahlou Workers, je souhaite un devis."));
  }, []);
  if (!href) return null;
  // Sans numéro configuré, on renvoie vers /contact plutôt qu'un faux numéro
  const fallback = href.includes("wa.me/") && href.endsWith("wa.me/") ? "/contact" : href;
  return (
    <a
      href={fallback}
      target={href === fallback ? undefined : "_blank"}
      rel="noreferrer"
      aria-label="Contacter Lahlou Workers sur WhatsApp"
      className="fixed bottom-5 right-5 z-40 flex h-14 w-14 items-center justify-center rounded-full border border-background/20 bg-primary text-primary-foreground shadow-lg transition-transform hover:-translate-y-1"
    >
      <Phone />
    </a>
  );
